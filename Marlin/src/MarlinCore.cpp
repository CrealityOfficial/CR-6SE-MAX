...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  TERN_(DWIN_CREALITY_SERMOON_V2, gLcdSermoonV2UI.AutoUIMainProcess());
  // Run i2c Position Encoders
  #if ENABLED(I2C_POSITION_ENCODERS)
  {
...
void setup() {
  ...
  // (because EEPROM code calls the UI).
    ...
    SETUP_RUN(dwinShow.RTS_Init());
    // SETUP_RUN(gLcdSermoonV2UI.AutoUIMainProcess());
  #else
    SETUP_RUN(ui.init());
  ...
  // #if BOTH(SDSUPPORT, USB_DISK_SUPPORT)
  //   SETUP_RUN(Sd2Card::config());          // U盘OTG_FS硬件初始化
  // #endif
  ...
  SETUP_RUN(stepper.init());          // Init stepper. This enables interrupts!
    ...
    SET_OUTPUT(E_MUX2_PIN);
      ...
      SET_INPUT(CHECK_MATWEIAL);  // 断料检测
    #endif
    SET_INPUT(CHECK_DOOR_PIN);    // 门开关
    SET_OUTPUT(RESET_WIFI_PIN);   // wifi复位
    SET_OUTPUT(LED_CONTROL_PIN);  // led
    SET_OUTPUT(BOX_FAN_PIN);      // 机箱风扇
    SET_OUTPUT(FAN_PIN);          // 散热风扇
  #endif
