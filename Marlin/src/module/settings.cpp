...
typedef struct SettingsDataStruct {
  ...
  //
  #if ENABLED(DWIN_CREALITY_SERMOON_V2)
    AutoUILanguage_t curLanguage;      // 语言
    uint32_t startupFirstTimeVal;      // 第一次上电
    uint32_t TotalRuningTime;          // 运行时间
    AutoUILevelStu_t autoLevelStatus;  // 自动调平状态
    __uint8_t wifiState;               // wifi使能
    float _zCoordinateOffset;          // Z轴空间坐标差
  #endif
} SettingsData;
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
    #if ENABLED(DWIN_CREALITY_SERMOON_V2)
      EEPROM_WRITE(gLcdSermoonV2UI.curLanguage);          // 语言
      EEPROM_WRITE(gLcdSermoonV2UI.startupFirstTimeVal);  // 第一次上电
      EEPROM_WRITE(gLcdSermoonV2UI.TotalRuningTime);      // 运行时间
      EEPROM_WRITE(gLcdSermoonV2UI.autoLevelStatus);      // 自动调平状态
      EEPROM_WRITE(gLcdSermoonV2UI.wifiCtrl.wifiState);   // wifi使能
      EEPROM_WRITE(gLcdSermoonV2UI.zCoordinateOffset);    // Z轴空间坐标差
    #endif
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
	  ...
	  #if ENABLED(DWIN_CREALITY_SERMOON_V2)
        EEPROM_READ(gLcdSermoonV2UI.curLanguage);         // 语言
        EEPROM_READ(gLcdSermoonV2UI.startupFirstTimeVal); // 第一次上电
        EEPROM_READ(gLcdSermoonV2UI.TotalRuningTime);     // 运行时间
        EEPROM_READ(gLcdSermoonV2UI.autoLevelStatus);     // 自动调平状态
        EEPROM_READ(gLcdSermoonV2UI.wifiCtrl.wifiState);  // wifi使能
        EEPROM_READ(gLcdSermoonV2UI.zCoordinateOffset);   // Z轴空间坐标差
      #endif
...
void MarlinSettings::reset() {
  ...
  //
  #if ENABLED(DWIN_CREALITY_SERMOON_V2)
    gLcdSermoonV2UI.ResetCurLang();           // ϵͳ����
    gLcdSermoonV2UI.ResetLevelStatus();       // ��ƽ״̬
    gLcdSermoonV2UI.ResetPrintTimeTotal();    // ������ʱ��
    gLcdSermoonV2UI.ResetStartFirstTimeVal(); // ��һ�ο�����־
    gLcdSermoonV2UI.ResetWifiStatus();        // wifi�ṹ�� 
    gLcdSermoonV2UI.ResetzCoordinateOffsetVal(); // �������ֵ
  #endif
