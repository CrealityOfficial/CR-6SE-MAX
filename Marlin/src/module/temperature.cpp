...
=======
      ...
      if (wait_for_heatup) {
        wait_for_heatup = false;
          ...
          gLcdSermoonV2UI.preheatFinishFlags.hotendHeat = true;
          if(printingIsActive() && (gLcdSermoonV2UI.AutoUIGetStatus() == DEVSTA_HEATING) && \
            (gLcdSermoonV2UI.preheatFinishFlags.bedHeat || !thermalManager.temp_bed.target))
          {
...
=======
      if (wait_for_heatup) {
        wait_for_heatup = false;
        gLcdSermoonV2UI.preheatFinishFlags.bedHeat = true;
        if(printingIsActive() && (gLcdSermoonV2UI.AutoUIGetStatus() == DEVSTA_HEATING) &&\
           gLcdSermoonV2UI.preheatFinishFlags.hotendHeat)
        {
          memset(&gLcdSermoonV2UI.preheatFinishFlags, 0, sizeof(gLcdSermoonV2UI.preheatFinishFlags));
