...
//IO?иC1??боии?????
#define SDA_IN()  {FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER&=~(3<<(3*2));FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER|=0<<3*2;}  //PA11
#define SDA_OUT() {FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER&=~(3<<(3*2));FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER|=1<<3*2;}  //pA11 
