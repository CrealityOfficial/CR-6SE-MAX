...
const PinName digitalPin[] = {
  ...
  PH_0,  // Digital pin 63, used by the external oscillator
  PH_1   // Digital pin 64, used by the external oscillator
};
...
static uint8_t SetSysClock_PLL_HSE(uint8_t bypass)
{
  ...
  RCC_OscInitStruct.PLL.PLLM            = 4;//8; // Expects an 8 MHz external clock by default. Redefine HSE_VALUE if not
  RCC_OscInitStruct.PLL.PLLN            = 72;//144;           // VCO output clock = 336 MHz (1 MHz * 336)
  RCC_OscInitStruct.PLL.PLLP            = RCC_PLLP_DIV2; // PLLCLK = 84 MHz (336 MHz / 4)
  RCC_OscInitStruct.PLL.PLLQ            = 3;//12;            // USB clock = 48 MHz (336 MHz / 7) --> OK for USB
